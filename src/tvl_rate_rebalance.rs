...
//! thresholds — higher TVL triggers a lower rate to preserve reward-token
...
//! "inside `stake` and `unstake` after position update" — this crate has no
...
//! `Vec` under one key rather than per-threshold entries — the issue caps
...
/// `current_tvl` — i.e. the tier `current_tvl` currently sits in. `None`
...
/// `set_reward_rate_bps` — introducing this feature never silently changes
...
pub fn check_and_rebalance(env: &Env) {
    ...
    let target_rate_bps = match active_threshold_for(&thresholds, current_tvl) {
        ...
        // a separately-stored "base" rate distinct from the live one, so —
        // matching how `set_reward_rate_bps` is the single source of truth
        // elsewhere — the lowest-TVL state simply leaves the current rate
...
impl VaultContract {
    ...
    /// it automatically — see this module's doc comment.
