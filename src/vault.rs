...
impl VaultContract {
    ...
    pub fn get_reputation_score(env: Env, user: Address) -> ReputationScore {
        let raw = Self::get_reputation_score_raw(&env, user.clone());
    ...
    pub fn anti_sybil_min_stake_age(
        ...
        let staked_at: u32 = env
            .storage()
            .instance()
            .get(&DataKey::StakedAtLedger(staker.clone()))
            .ok_or(VaultError::BelowMinimumStake)?;
        let current_ledger = env.ledger().sequence();
    ...
    pub fn propose_merger(
        ...
        let key = (symbol_short!("mrg_prop"), source_pool.clone());
    ...
    pub fn execute_merger(
        ...
        let key = (symbol_short!("mrg_prop"), source_pool.clone());
        let _: Address = env.storage().persistent()
            .get(&key)
    ...
    pub fn set_branding(
        ...
        if config.display_name.len() > MAX_BRANDING_NAME_LEN {
            return Err(VaultFeatureError::InvalidBrandingField);
        }
    ...
    pub fn set_insurance_product(
        ...
        if premium_bps > BOOST_BPS_BASE || max_coverage_per_user < 0 {
            return Err(VaultFeatureError::InvalidInsuranceConfig);
        }
    ...
    pub fn open_prediction_market(
        ...
        if target_ledger <= env.ledger().sequence() {
            return Err(VaultFeatureError::InvalidLotSize);
        }
    ...
    pub fn place_bet(
        ...
        let mut market: PredictionMarket = env
            ...
            .ok_or(VaultFeatureError::PositionNotFound)?;
    ...
    pub fn claim_prediction_winnings(
        ...
        let (bet_higher, bet_amount): (bool, i128) = env
            ...
            .instance()
            .get(&bet_key)
            .ok_or(VaultFeatureError::RevenueShareAlreadyClaimed)?;
