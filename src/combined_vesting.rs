...
//! is applied to — `vesting_cliff::apply_cliff` is not called from here.
//!
//! # Wiring
//!
...
//! and read-only queries — mirroring the level of integration already
...
/// `vesting_cliff.rs` does — `balance.rs` exposes no accessor for it.
...
/// leg — cliff unlock plus the linear period.
...
///
/// - Before the cliff: `(0, raw_reward)` — nothing claimable yet.
...
/// - After the linear period ends: `(raw_reward, 0)` — fully vested.
fn split_vested(
    env: &Env,
    config: &CombinedVestingConfig,
    staked_at: u32,
    ...
    // raw_reward * elapsed / period — checked, since a hostile-sized reward
...
/// claimable) when no combined config is set — same "off by default"
